        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="md:col-span-2">
            ...
            <p className="text-sm text-gray-400 leading-relaxed max-w-xs">
              Your trusted platform for Event tickets. Play responsibly and may luck be on your side.
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {[['about', 'About Us'], ['terms', 'Terms & Conditions'], ['privacy', 'Privacy Policy'], ['refunds_cancellations', 'Refunds & Cancellations']].map(([type, label]) => (
                <li key={type}><Link to={`/page/${type}`} className="text-sm text-gray-400 hover:text-white transition-colors">{label}</Link></li>
              ))}
            </ul>
          </div>
        </div>
